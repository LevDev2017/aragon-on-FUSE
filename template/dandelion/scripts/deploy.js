...
async function main() {
  ...
  const DandelionOrg = await hre.ethers.getContractFactory("DandelionOrg");
  const dandelionOrg = await DandelionOrg.deploy(_daoFactory, _ens, _miniMeFactory, _aragonID);
  await dandelionOrg.deployed();
  console.log("DandelionOrg deployed to:", dandelionOrg.address);

  const writeContractsAddr = "DAOFactory=" + _daoFactory + "\nENS=" + _ens + "\nMiniMeTokenFactory=" + _miniMeFactory + "\nFIFSResolvingRegistrar="+ _aragonID + "\nDandelionOrg="+ dandelionOrg.address;
