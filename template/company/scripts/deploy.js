...
async function main() {
  ...
  const CompanyTemplate = await hre.ethers.getContractFactory("CompanyTemplate");
  const companyTemplate = await CompanyTemplate.deploy(_daoFactory, _ens, _miniMeFactory, _aragonID);
  await companyTemplate.deployed();
  console.log("CompanyTemplate deployed to:", companyTemplate.address);

  const writeContractsAddr = "DAOFactory=" + _daoFactory + "\nENS=" + _ens + "\nMiniMeTokenFactory=" + _miniMeFactory + "\nFIFSResolvingRegistrar="+ _aragonID + "\nDandelionOrg="+ companyTemplate.address;
