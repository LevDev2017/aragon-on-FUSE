...
async function main() {
  ...
  const MembershipTemplate = await hre.ethers.getContractFactory("MembershipTemplate");
  const membershipTemplate = await MembershipTemplate.deploy(_daoFactory, _ens, _miniMeFactory, _aragonID);
  await membershipTemplate.deployed();
  console.log("MembershipTemplate deployed to:", membershipTemplate.address);
