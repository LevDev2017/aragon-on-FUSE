...
async function main() {
  ...
  const ReputationTemplate = await hre.ethers.getContractFactory("ReputationTemplate");
  const reputationTemplate = await ReputationTemplate.deploy(_daoFactory, _ens, _miniMeFactory, _aragonID);
  await reputationTemplate.deployed();
  console.log("ReputationTemplate deployed to:", reputationTemplate.address);

  const writeContractsAddr = "DAOFactory=" + _daoFactory + "\nENS=" + _ens + "\nMiniMeTokenFactory=" + _miniMeFactory + "\nFIFSResolvingRegistrar="+ _aragonID + "\nReputationTemplate="+ reputationTemplate.address;
